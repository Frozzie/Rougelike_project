...
namespace XYZRoguelike
{
    ...
    void MazeGenerator::Generate()
    {
        ...
        // Use a stack to keep track of points where the enemies would spawn, and spawn them after the maze generation is
        // done
        std::stack<std::pair<int, int>> stackMem;
        ...
        while (!stack.empty())
        {
            ...
            if (!directions.empty())
            {
                ...
                // or with a small chance save current position to use later to spawn roamers in, so they wont "dissapear"
                // when moving between tiles
                int chance = rand() % 10;
                if (chance == 0)
                {
                    //
    ...
    void MazeGenerator::RemoveWall(int x1, int y1, int x2, int y2)
    {
        ...
        {
            level->walls.push_back(std::make_unique<Wall>(XYZEngine::Vector2Df{ wallX * 128.f, wallY * 128.f },
                14)); // Wall is being crated here (use for spawning enemies)
        }
    }
}
